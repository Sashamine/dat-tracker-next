...
function summarize(report) {
    ...
    for (const r of failing) {
        for (const iss of [...r.hardIssues, ...r.warnIssues])
            topIssues.set(iss, (topIssues.get(iss) ?? 0) + 1);
    }
    ...
    if (failing.length) {
        ...
        if (failing.length > 30)
            lines.push(`- ...and ${failing.length - 30} more`);
        lines.push('');
...
async function main() {
    ...
    for (const ticker of tickers) {
        ...
        if (j.schemaVersion !== '0.1')
            issues.push('schemaVersion_not_0.1');
        // evidence_quality
        // Flag low-quality evidence URLs (index pages) so we can upgrade to exact links over time.
        const isLowQualityEvidenceUrl = (u, isUsLike) => {
            if (typeof u !== 'string')
                return false;
            const s = u.toLowerCase();
            // obvious index/landing pages
            if (s.includes('cgi-bin/browse-edgar') || s.endsWith('/investors') || s.includes('/investor-relations'))
                return true;
            if (!isUsLike)
                return false;
        ...
        if (hs2 === 'sec-filing') {
            ...
            if (!url && !acc)
                issues.push('missing_sec_holdings_evidence');
        }
        ...
        if (j.ticker !== ticker)
            issues.push('ticker_mismatch');
        if (!j.asOf)
            issues.push('missing_asOf');
        if (!j.generatedAt)
            issues.push('missing_generatedAt');
        if (!j.listing?.country)
            issues.push('missing_listing.country');
        if (!j.listing?.exchangeMic)
            issues.push('missing_listing.exchangeMic');
        if (!j.listing?.currency)
            issues.push('missing_listing.currency');
        if (!j.holdings?.asset)
            issues.push('missing_holdings.asset');
        if (typeof j.holdings?.amount !== 'number' || Number.isNaN(j.holdings?.amount))
            issues.push('invalid_holdings.amount');
    ...
    for (const r of results) {
        const all = [...r.hardIssues, ...r.warnIssues];
        for (const iss of all) {
            (gaps[iss] ||= []).push(r.ticker);
        }
    }
