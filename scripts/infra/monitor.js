...
const ageMinutes = (isoOrNull, nowMs) => {
  if (!isoOrNull) return null;
  const t = Date.parse(isoOrNull);
  if (!Number.isFinite(t)) return null;
  return Math.floor((nowMs - t) / 60000);
};
const ageMinutesFromEpochSeconds = (sec, nowMs) => Math.floor((nowMs - sec * 1000) / 60000);
...
let meaningfulCommitAgeMin = null;
try {
...
if (st.mode === 'running') {
  ...
  if (progAge >= STALE_MIN && mcAge >= STALE_MIN) alerts.push(`running-but-stale>=${STALE_MIN}m`);
  if (dirtyCount > 0 && lastCommitAgeMin >= DIRTY_SOFT_MIN) alerts.push(`dirty-tree>=${DIRTY_SOFT_MIN}m`);
  if (dirtyCount > 0 && lastCommitAgeMin >= STALE_MIN) alerts.push(`dirty-tree>=${STALE_MIN}m`);
  if ((updatedAgeMin ?? VERY_STALE_MIN) >= VERY_STALE_MIN && lastCommitAgeMin >= VERY_STALE_MIN) alerts.push(`possible-hidden-block>=${VERY_STALE_MIN}m`);
}
...
if (alerts.length) console.log(`ALERT ${alerts.join(',')} | ${base}`);
else console.log(`OK | ${base}`);
