...
async function extractWithOpenAI(params: {
    ...
    const system = `You extract stock share corporate actions from filing text. Return ONLY valid JSON (no markdown). Only include events that are explicitly stated in the text. A corporate action must include an exact supporting QUOTE copied verbatim from the text.`;
    const user = {
...
async function main() {
  ...
  const counts = await d1.query<{ source_type: string; cnt: number }>(
        `SELECT source_type, COUNT(1) as cnt FROM artifacts WHERE source_type IN ('hkex_pdf', 'sedar_filing', 'sec_filing') AND ticker IS NOT NULL GROUP BY source_type ORDER BY cnt DESC;`
      );
  ...
  for (const a of artifacts.results) {
      ...
      for (const ca of extracted) {
          ...
          const near = await d1.query<{ cnt: number }>(
                    `SELECT COUNT(1) as cnt FROM corporate_actions
                             WHERE entity_id = ? AND action_type = ? AND ratio = ?
