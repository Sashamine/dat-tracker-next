...
while IFS= read -r t; do
  [[ -z "$t" ]] && continue
  out=$(node "$ROOT_DIR/scripts/sec/apply-debt-from-companyfacts.js" "$t" 2>/tmp/sec_debt_err.txt || true)
  echo "$out"

  if grep -q "fetch_failed:404" /tmp/sec_debt_err.txt; then
    node "$ROOT_DIR/scripts/sec/dlq-push.js" "{\"kind\":\"sec_companyfacts_404\",\"at\":\"$(date -u +%Y-%m-%dT%H:%M:%SZ)\",\"ticker\":\"$t\",\"mode\":\"debt\"}" || true
...
if [[ $dataChanged -eq 1 ]]; then
  ...
  (cd "$ROOT_DIR" && npm run -s state:verify:compiled:auto -- --runId debt-extract-batch)
  ...
  (cd "$ROOT_DIR" && git commit -m "chore(sec): apply debt companyfacts batch" || true)
  ...
  if [[ ! -f "$DLQ_THROTTLE_FILE" ]]; then
    ...
    (cd "$ROOT_DIR" && git commit -m "chore(sec): update debt DLQ" || true)
