...
async function main() {
  ...
  const keepSet = new Set(
    keep
      ? keep.split(',').map(s => s.trim()).filter(Boolean)
      : []
  );
  ...
  console.log({ artifactId, dryRun, keep: Array.from(keepSet), metrics: rows.results });

  if (!keepSet.size) {
    console.error('Refusing to delete without --keep=...');
    process.exit(1);
  }
